 ...
 * Lógica del juego.
 * 
 * Para mostrar el tablero se debe añadir el constructor y el método abajo
 * indicado
 * 
...
//		TableroController construirTablero = new TableroController();
		do {
			...
			comprobarPosicion(celda, fila, columna, jugador);
			...
			if (tablero.getCoordenadaFilaIni() == fila && tablero.getCoordenadaColumnaIni() == columna
					&& jugador.isTieneOro()) {
				seguir = false;
				estadoPartida = true;
				System.out.println(mensaje.MENSAJE_ORO_SALIDA);
			}
			if (seguir) {
...
//			construirTablero.mostrarTablero(tablero);
	...
	@SuppressWarnings("static-access")
	private void comprobarPosicion(Celda[][] celda, int fila, int columna, Jugador jugador) {
		...
		if (celda[fila][columna].isOroHabilitado()) {
			System.out.println(mensaje.MENSAJE_ORO_DETECTADO);
